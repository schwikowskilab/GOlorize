...
class MouseLayoutPanelHandler extends MouseAdapter{
    ...
    public void mouseClicked(MouseEvent ev){ 
        ...
        if (/*jTable1.columnAtPoint(point)*/modelColumn==layoutPanel.getGoTermColumn()) {
            ...
            if (SwingUtilities.isLeftMouseButton(ev) ){
                ...
                try {
                	...
                	fr.systemsbiology.golorize.internal.BiNGO.Browser.init(); ///A REFAIRE CHOIX PAS PREVU + NOUVELLE FENETRE A CHAQUE FOIS ??
                    //if (jTable1.getColumnName(1)=="GO-ID : SGD")
                    //     Browser.displayURL(urlSGD+GOid);
                    //else 
                	fr.systemsbiology.golorize.internal.BiNGO.Browser.displayURL(urlGO+GOid);
        ...
        if (modelColumn==layoutPanel.getDescriptionColumn()){
            ...
            layoutPanel.getGOlorize().getGoColor().put(jTable1.getValueAt(jTable1.rowAtPoint(point),ResultPanel.GO_TERM_COLUMN),newColor);
            layoutPanel.getGOlorize().synchroColor();
